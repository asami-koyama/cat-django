...
const CatEdit = () => {
  ...
  const [values, setValues] = useState({
    ...
    pattern: { value: row.pattern, headerName: '柄' },
    color: { value: row.color, headerName: '色' },
    note: { value: row.note, headerName: '備考' },
    image: { value: row.img, previewImage: row.img, isUpdate: false },
  });
  ...
  const registerData = () => {
    ...
    // 新しい画像をアップロードする場合
    if (values.image.value && values.image.isUpdate) {
    ...
    axios
      .patch(`http://192.168.150.201:8000/api/catsRUD/` + id + '/', form_data, {
        headers: {
      ...
      >
        ...
        >
          ...
          >
            ...
            <Grid container alignitems="center" justify="center" sx={{ mt: 2 }}>
